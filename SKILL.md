...
由 Windows 任务计划程序「PendingInvoiceChecker」驱动（**用户登录时触发 + 每 2 小时重复，持续 1 天**），通过 `scripts/run_scheduled.ps1` 调用 `codebuddy` CLI 无头模式执行 `SCHEDULED_TASK.md`：

- **触发时机**：用户登录 Windows 时立即触发一次，之后每 2 小时重复
- **时段守卫**：工作日（周一至周五）9:00–18:00 才执行，其余静默退出
- **执行**：完整四阶段流程（同 `/pending-invoice-checker`）
- **登录引导（阻塞补跑）**：兴趣岛/企微未登录 → 弹「需登录」引导窗（**同步阻塞**等用户点确定）→ 用户登录后点确定 → **立即补跑**，最多重试 3 次；仅非登录类异常才停止本次
- **弹窗**：0 条静默；>0 条弹「N 条 + 报告路径」；失败弹告警
- **报告**：有订单才生成，历史保留

任务指令见 `SCHEDULED_TASK.md`，弹窗通过 `scripts/popup.ps1` 实现。详见《pending-invoice-checker-定时任务技术方案.md》。
...
├── SKILL.md                        ← 本文件
├── SCHEDULED_TASK.md               ← 定时任务指令(登录触发+每2小时)
├── references/
